...
<fieldset style="width: auto"><legend class="e6" >Certificado laboral</legend>	
	<c:if test="${sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 1 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 214 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 235}">
		 <span class="enlace" id='lnkUpdateUsuarioDetalle' name='lnkUpdateUsuarioDetalle'>Actualizar</span>&nbsp;&nbsp;&nbsp;
				...
				<div class="texto1">1. La contraseña para abrir el certificado es su número de cedula.</div> 
				<br>
				<div class="texto1">2. Recuerde que solo puede generar un máximo de  cinco(5) certificados por mes.</div>
				<br> 
				...
				<div class="texto1"> Para gestionar la recuperación de su contraseña, debe conocer las respuestas a las preguntas de seguridad  que eligió, esta información la puede actualizar desde el link  "cambiar contraseña", que se encuentra  en la parte superior derecha, en la pestaña "preguntas de seguridad".
						Cuando olvide su clave, en la página de ingreso en la parte inferior encontrara el link "recordar contraseña" para iniciar el proceso de recuperación.
				</div>
	</c:if>
</fieldset>
</div>
